const answer1 = confirm('JavaScript появился в 1995 году?');

if (answer1 === true) {
    alert('Верно');
} else {
    alert('JavaScript действительно появился в 1995 году');
}

const answer2 = confirm('Спецификация JavaScript называется ECMAScript?');

if (answer2 === true) {
    alert('Верно');
} else {
    alert('Да, спецификация JavaScript называется ECMAScript');
}

const answer3 = confirm('JavaScript был создан за 1 месяц?');

if (answer3 === true) {
    alert('Верно');
} else {
    alert('Нет, JavaScript был создан примерно за 10 дней Бренданом Эйхом');
}